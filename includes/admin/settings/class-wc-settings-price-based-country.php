...
if ( ! class_exists( 'WC_Settings_Price_Based_Country' ) ) :
	...
	class WC_Settings_Price_Based_Country extends WC_Settings_Page {
		...
		public function enqueue_scripts() {
			if ( ! $this->is_section( array( 'edit-zone', 'options', 'zone-list', 'free-pro' ) ) ) {
				return;
			}
		...
		protected function output_zones_list_table() {
			?>
			<h3><?php esc_html_e( 'Pricing zones', 'woocommerce-product-price-based-on-countries' ); ?>&nbsp;
				<a class="add-new-h2" href="<?php echo esc_url( admin_url( 'admin.php?page=wc-settings&tab=price-based-country&zone_id=new' ) ); ?>">
				<?php esc_html_e( 'Add pricing zone', 'woocommerce-product-price-based-on-countries' ); ?>
				</a>
			</h3>
