...
class WCPBC_Frontend {
	...
	public static function maybe_refresh_cart() {
		if ( version_compare( WC_VERSION, '3.6', '<' ) || ( defined( 'DOING_AJAX' ) && DOING_AJAX ) || is_cart() || is_checkout() || ! isset( $_COOKIE['woocommerce_cart_hash'] ) || ! did_action( 'woocommerce_cart_loaded_from_session' ) || did_action( 'woocommerce_after_calculate_totals' ) ) {
			return;
		}
		$cart_hash   = WC()->cart->get_cart_hash();
		$cookie_hash = isset( $_COOKIE['woocommerce_cart_hash'] ) ? $_COOKIE['woocommerce_cart_hash'] : ''; // phpcs:ignore
